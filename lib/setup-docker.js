...
async function run() {
  ...
  if (platform === 'darwin') {
    ...
    await exec.exec('docker', [
      'compose', 'version']).catch(() => { });

    core.startGroup('download docker')
    ...
    await exec.exec('mkdir', [
      '-p',
      '/Users/runner/.docker/cli-plugins'
    ]);

    await exec.exec('cp', [
      'buildx-v0.9.0-rc1.darwin-amd64',
...
`]);
    ...
    await exec.exec('sudo', ['docker','version']);
    core.endGroup();

    core.startGroup('docker info');
    await exec.exec('sudo', ['docker','info']);
    core.endGroup();
  ...
  if (DOCKER_CHANNEL === 'nightly') {
    ...
    core.endGroup();
    core.startGroup('install docker-compose-plugin(compose v2)');
    await exec.exec('sudo', [
      'apt-get',
      '-y',
      'install',
